...
public extension ImageSource {
    ...
    func updateTargetsWithFramebuffer(_ framebuffer: Framebuffer, shouldKeepRenderFramebufferIfNoTarget: Bool = false) {
        ...
        if foundTargets.count == 0 { // Deal with the case where no targets are attached by immediately returning framebuffer to cache
            if !shouldKeepRenderFramebufferIfNoTarget {
                framebuffer.lock()
                framebuffer.unlock()
            }
        } else {
...
public extension ImageConsumer {
    ...
    func removeAllSources() {
        ...
        sources.sources.forEach { $0.value.remove(self) }
...
public class WeakImageConsumer {
    public private(set) weak var value: ImageConsumer?
...
extension OperationGroup {
    public var debugPipelineDescription: String {
        ...
        if let obj = self as? DebugPipelineNameable {
            return "\(obj.debugNameForPipeline) -> \(outputImageRelay.debugPipelineDescription)"
        }

...
public class PictureInput: ImageSource {
    ...
    public var shouldSkipProcessingIfRendering = false
...
"""
    ...
    public convenience init(image: UIImage, imageName: String = "UIImage", smoothlyScaleOutput: Bool = false, orientation: ImageOrientation? = nil, processSteps: [PictureInputProcessStep]? = nil) throws {
        #if DEBUG
        let startTime = CACurrentMediaTime()
        #endif
        ...
        } else if image.cgImage == nil, let ciImage = image.ciImage, let rotatedImage = ciImage.renderToCGImage(onGPU: false) {
            croppedCGImage = rotatedImage
        } else {
...
"""
        ...
        try self.init(image: cgImage, imageName: imageName, smoothlyScaleOutput: smoothlyScaleOutput, orientation: targetOrientation, preprocessRenderInfo: preprocessRenderInfo)
    }
    ...
    public func processImage(synchronously: Bool = false) {
        if shouldSkipProcessingIfRendering && frameRenderingSemaphore.wait(timeout: DispatchTime.now()) != DispatchTimeoutResult.success {
            return
        }
        
        self.imageFramebuffer?.userInfo = self.framebufferUserInfo
        
        sharedImageProcessingContext.runOperation(sync: synchronously) {
            ...
            #endif
            if self.shouldSkipProcessingIfRendering == true {

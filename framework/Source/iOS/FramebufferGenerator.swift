...
public class FramebufferGenerator {
    ...
    public func generateFromYUVBuffer(_ yuvPixelBuffer: CVPixelBuffer, frameTime: CMTime, processSteps: [PictureInputProcessStep]? = nil, videoOrientation: ImageOrientation) -> Framebuffer? {
        var framebuffer: Framebuffer?
        sharedImageProcessingContext.runOperationSynchronously {
            framebuffer = _generateFromYUVBuffer(yuvPixelBuffer, frameTime: frameTime, processSteps: processSteps, videoOrientation: videoOrientation)
        }
        return framebuffer
    }
...
private extension FramebufferGenerator {
    ...
    func _generateFromYUVBuffer(_ yuvPixelBuffer: CVPixelBuffer, frameTime: CMTime, processSteps: [PictureInputProcessStep]?, videoOrientation: ImageOrientation) -> Framebuffer? {
//        let startTime = CACurrentMediaTime()
        ...
        if var processSteps = processSteps, !processSteps.isEmpty {
            var ciImage = CIImage(cvPixelBuffer: yuvPixelBuffer)
            ...
            processSteps.append(.scale(x: 1, y: -1, anchorPoint: .extentCenter))
            ciImage = ciImage.processed(with: processSteps)
            framebuffer.activateFramebufferForRendering()
            CIImage.ciGPUContext.draw(ciImage, in: CGRect(origin: .zero, size: ciImage.accurateExtent.rounded(.towardZero).size), from: ciImage.accurateExtent.rounded(.towardZero))
        } else {

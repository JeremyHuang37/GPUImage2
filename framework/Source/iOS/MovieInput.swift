...
public class MovieInput: ImageSource {
    ...
    // realtime process steps based on current frame time
    public var timeBasedProcessSteps: ((CMTime) -> [PictureInputProcessStep]?)?
    ...
    func process(movieFrame: CVPixelBuffer, withSampleTime: CMTime) {
        ...
        if let processSteps = processSteps(at: withSampleTime), !processSteps.isEmpty {
...
"""
    ...
    func processSteps(at displayTime: CMTime) -> [PictureInputProcessStep]? {
        let extraProcessSteps = timeBasedProcessSteps?(displayTime)
